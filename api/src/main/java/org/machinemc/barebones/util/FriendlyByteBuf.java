...
public class FriendlyByteBuf extends DuplicatedByteBuf {
     ...
     */
    @Contract("_ -> this")
    public <T extends Writable> FriendlyByteBuf write(final T writable) {
        writable.write(this);
        return this;
    }
    ...
    public String readString(final Charset charset) {
        ...
        final int length = readVarInt();
        if (length < 0) throw new IllegalStateException(STR."String has illegal length of: \{length}");
    ...
    public <T extends Enum<T>> T readEnum(final Class<T> enumClass) {
        ...
        if (index >= constants.length)
            throw new IndexOutOfBoundsException(STR."Index \{index} is out of bounds for enum \{enumClass.getName()}");
